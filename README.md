...
- ### Design
  ...
  - #### Imagery
    - Imagery used to make up the logo was taken from [Unsplash](https://unsplash.com) which was a side view of water through glass and then using [GIMP](https://www.gimp.org/) a letter 'B' was imposed on the image.
  - #### Wireframe
    - This wireframe was used initially to get an idea of how the style and layout of the app would be:
...
- [JSX](https://reactjs.org/docs/introducing-jsx.html) is used in [React](https://reactjs.org/), a Javascript library,
  which generates the [HTML](https://en.wikipedia.org/wiki/HTML5) used in the project.
- [Python](https://en.wikipedia.org/wiki/Python_(programming_language))

### Frameworks, Libraries & Programs Used

1. [Google Fonts:](https://fonts.google.com/)
   - Google fonts were used to import the 'Open Sans' font into the style.css file which is used in all text on the page. Google fonts Material symbols icons was also used for the search information icon and the drawer tab double arrow icon.
...
1. [Django Rest Framework:](https://www.django-rest-framework.org/)
   - Django Rest Framework was used to create the Rest API to make requests to the Postgresql database as well as middleware to make requests to the Google Places API. Its was also used for the authentication, authorization and permissions.
...
1. [Deckgl:](https://deck.gl/)
   - Deckgl was used for the station column layers and view state.
1. [Mapbox:](https://www.mapbox.com/)
   - Mapbox was used for the geo visualisation map.
...
- #### First Time Visitor Goals
  ...
  1. As a First Time Visitor, I want to easily create an account and log in so that I can update fuel station prices.
     1. when the user is not loggin in as when visiting for the first time, there is a message to explain that you need to create an account and log in if the user wants to update fuel prices.
     2. On first visit the UI displays inputs for registration.
     3. If the registration is successful the app will automatically log you in so that you can update fuel prices.
  2. As a First Time Visitor, I want to be able to find the cheapest fuel prices in a given area without needing to login so that I know where the cheapest fuel can be bought.
     ...
     3. Deckgl will geo locate to the coordinates found after a successful search and display an error message if it is not successful.
     4. The map shows two columns for each station an on hover tooltip with a legend for petrol and diesel prices for that station.

- #### Returning Visitor Goals

  1. As a Returning Visitor, I want to be able to log in to be able to update fuel station the prices.
     1. If the user is not logged in and the previous sessions is expired the user will be shown the login and register section in the UI.
...
|----------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|
| Check search area responds when user clicks 'Go' with correct/incorrect/no input.  | Sidebar will close upon hitting go and the map will move to the place entered and display any fuels stations that may be with 3km.       An error message should be displayed if there is no or incorrect input.      If the user hovers or clicks in the information icon in the search area section then they will see a tooltip explaining how the search works.                                                                                                                                                                                                                                                                                                                                                                                                                | As expected    |
...
| Register section should give appropriate feedback if incorrect input is used, or register and login if not.    | If a username or email address has been used previously, or if the passwords do not match, or if the password is invalid an error message should be displayed to the user.       If any of the fields is empty the browser should display a message to indicate that the field is required.       The UI should log the user in automatically upon successful registration.       Once logged in the logout section should be displayed.                                                                                                                                                                                                                                                                                                                                           | As expected    |
...
| Test update station section.                                                     | The update section should only appear when the user is logged in and when a station on the map is clicked on.       Petrol and diesel price inputs should only accept numerical input and both fields should be filled out.       If both fields are not filled out the browser should display a message to fill in required fields.       If non numerical data is entered an error message should be displayed.       If the station prices are successfully updated the date updated and the user who updated it should be displayed in the update station section and in the map tooltip, and the map columns should update.      If the station price information is deleted there should be a warning message and if accepted the map columns and map tooltip should update. | As expected    |
| Sidebar drawer open and close function.                                          | If the user clicks on the top right tab then the drawer should open or close with the arrow indicating action.     If the user is logged in and clicks on a station then the drawer should open if not already.       If the app was loaded on a device with a width less than 1024px and is user not logged in, the drawer will close when a station is clicked on.       If the user does an area search then the drawer will close if the search is successful.                                                                                                                                                                                                                                                                                                                 | As expected    |
| Map tooltip display and information.                                             | A map tooltip should be displayed on hover (or on tap for touch devices).  It should display information on the station name, address, if it is  currently open according to Google Places API, the petrol and diesel prices, when they were updated and by which user.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | As expected    |
...
- Javascript
  - I can confirm that there were no ESLint errors. Warnings were only related to useEffect dependencies which would cause an infinite rerendering of the components if the suggestions was followed.
  ![ESLint](./assets/images/eslint.png)
- Accessibility

  - I confirm that the colours and fonts are easy to read and accessible by running it through Lighthouse in devtools on Chrome.

  ![LIGHTHOUSE METRICS](./assets/images/lighthouse-blugold.png)

## Deployment

- Local deployment during development was achieved by writing a catchall view that proxies requests for index.html to the React development server using this tutorial: [Making React and Django play well together](https://fractalideas.com/blog/making-react-and-django-play-well-together-hybrid-app-model/).
